...
const App: React.FC = () => {
  ...
  const processEvents = (events: any[]) => {
    events.forEach((e) => {
      ...
      if (e.type === "ATTACK" && e.data?.targetId) {
        const target = engineRef.current.entities.find(
          (ent) => ent.id === e.data.targetId,
        );
        if (target) {
  ...
  const handleAction = async (action: GameAction) => {
    ...
    if (action.type !== "LOOK" && action.type !== "WAIT") {
      void 0; // no-op to satisfy no-empty rule
    }
  ...
  const executeCommand = async (rawCmd: string, isAiParsed = false) => {
    ...
    const getDir = (s: string) => {
      if (["n", "north", "up", "с", "север"].includes(s)) {
        return { dx: 0, dy: -1 };
      }
      if (["s", "south", "down", "ю", "юг"].includes(s)) {
        return { dx: 0, dy: 1 };
      }
      if (["w", "west", "left", "з", "запад"].includes(s)) {
        return { dx: -1, dy: 0 };
      }
      if (["e", "east", "right", "в", "восток"].includes(s)) {
        return { dx: 1, dy: 0 };
      }
      return null;
    };
    ...
    if (
      dir &&
      (["move", "go", "идти", "n", "s", "e", "w", "с", "ю", "в", "з"].includes(
        verb,
      ) ||
        dir)
    ) {
    ...
    switch (verb) {
      ...
      case "удар": {
        ...
        if (args.length > 0) {
          ...
          const t = engineRef.current.entities.find(
            (e) => e.label === label && !e.isDead,
          );
          ...
          const t = engineRef.current.entities.find(
            (e) =>
              e.isHostile && !e.isDead && Math.abs(e.pos.x - p.pos.x) <= 1.5,
          );
        ...
        if (targetId) {
          await handleAction({ type: "ATTACK", payload: { targetId } });
        } else {
          addLog("Кого атаковать?", LogType.ERROR);
        }
      ...
      case "пить":
        if (args.length > 0) {
          await handleAction({
            type: "USE",
            payload: { itemName: args.join(" ") },
          });
        } else {
          addLog("Что использовать?", LogType.ERROR);
        }
        break;
      ...
      case "говорить": {
        ...
        if (!text) {
          addLog("Что сказать?", LogType.ERROR);
          break;
        }
      ...
      case "инвентарь": {
        ...
        items.forEach((i) => addLog(`- ${i.name}`, LogType.INFO));

        if (items.length === 0) {
          addLog("Пусто.", LogType.INFO);
        }
  ...
  const ShortcutBtn = ({
    ...
    <button
      ...
      className={`${color} hover:brightness-110 text-gray-200 text-xs py-2 px-3 rounded font-mono border border-neutral-700`}
    >
      {label}
    </button>
  );
  ...
  const getContextShortcuts = () => {
    ...
    const nearbyEnemies = entities.filter(
      (e) => e.isHostile && !e.isDead && Math.abs(e.pos.x - p.pos.x) <= 1.5,
    );
    const nearbyNPC = entities.find(
      (e) => e.type === EntityType.NPC && Math.abs(e.pos.x - p.pos.x) <= 1.5,
    );
    const itemUnderfoot = entities.find(
      (e) =>
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-2">
        ...
        {nearbyNPC && (
          <>
            <ShortcutBtn
              label={`ГОВОРИТЬ (${nearbyNPC.name})`}
              cmd="говорить"
              color="bg-blue-900 border-blue-600"
            />
            {nearbyNPC.npcType === "MERCHANT" && (
              <ShortcutBtn
                label={`КУПИТЬ (50g)`}
                cmd="купить"
        ...
        {itemUnderfoot && itemUnderfoot.type === EntityType.ITEM && (
          <ShortcutBtn
            label="ВЗЯТЬ"
            cmd="взять"
            color="bg-cyan-900 border-cyan-600 font-bold"
          />
        )}
        {itemUnderfoot &&
          itemUnderfoot.type === EntityType.EXIT &&
        ...
        {p.inventory && p.inventory.some((i) => i.type === ItemType.POTION) && (
          <ShortcutBtn
            label="Пить Зелье"
            cmd="пить зелье"
  ...
  if (!world || !player) {
    return <div className="text-white p-10">Загрузка Engine...</div>;
  }

  const currentTarget = targetLabel
    ? entities.find((e) => e.label === targetLabel && !e.isDead)
    : null;
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 bg-black flex flex-col relative border-r border-neutral-800">
          ...
          <div className="absolute bottom-4 left-4 grid grid-cols-3 gap-1 opacity-50 hover:opacity-100 transition-opacity">
            <div></div>
            <ShortcutBtn label="С" cmd="идти север" />
            <div></div>
            <ShortcutBtn label="З" cmd="идти запад" />
            <ShortcutBtn label="WAIT" cmd="ждать" />
            <ShortcutBtn label="В" cmd="идти восток" />
            <div></div>
            <ShortcutBtn label="Ю" cmd="идти юг" />
            <div></div>
          </div>
        </div>
        <div className="w-[450px] flex flex-col bg-neutral-900">
          <div className="flex-1 overflow-hidden relative">
            <GameLog logs={logs} />
            {aiLoading && (
              <div className="absolute bottom-2 right-2 text-xs text-cyan-500 animate-pulse bg-black px-2 py-1 rounded border border-cyan-900">
                ... AI Думает ...
              </div>
            )}
          </div>
          ...
          <div className="p-3 bg-neutral-950 border-t border-neutral-800">
            ...
            <div className="text-[10px] text-gray-600 mt-1 flex justify-between">
              <span>
                {world.level === 0 ? "Город" : `Уровень ${world.level}`}
              </span>
            </div>
          </div>
        </div>
